...
class image_converter:
    ...
    def __init__(self):
        ...
        self.joint1_pub = rospy.Publisher("joints_pos/joint_angle_1", Float64, queue_size=10)
    ...
    def detect_joint_angles(self):
        ...
        yellowxy = np.array([yellow[0],yellow[1]])
        bluexy = np.array([blue[0],blue[1]])

        vec = bluexy-yellowxy
        y2 = np.array([0,1])
        ja1 = self.angle(vec,y2)-np.pi

        ja3 = self.angle(link2, y) - np.pi/2 - 0.15
        ja3 = -ja3
        ...
        ja4 = self.angle(link2, link3)-0.15
        if ja4 > np.pi / 2:
            ja4 = np.pi-ja4
        if ja4 < (- np.pi / 2):
            ja4 = ja4 + np.pi
        project = np.dot(link3b, crossprod)
        if project < 0:
            ja4 = -ja4
